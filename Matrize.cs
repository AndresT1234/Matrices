...
namespace Matricez
{
    class Matriz
    {
        ...
        public Matriz Potencia(int potencia)
        {
            ...
            if (potencia < 0)
            {
                // Si la potencia es negativa, calculamos la inversa de la matriz
                Matriz inversa = CalcularInversa();
                return inversa.Potencia(-potencia);
            }

            Matriz resultado = new Matriz(GetFilas(), GetColumnas());
            Matriz baseMatriz = CopiarMatriz(this);
